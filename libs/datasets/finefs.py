...
class FineFSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 构建数据字典，每个字段含义如下：
        data_dict = {
            'video_id'        : video_item['id'],         # 视频唯一标识符（字符串），用于区分不同视频
            'feats'           : feats,                    # 视频特征，形状为 C x T，C为特征维度，T为特征序列长度（时间步/clip数）
            'segments'        : segments,                 # 动作实例的区间，形状为 N x 2，N为动作数量，每行为[start, end]（特征在feats中的坐标）
            'labels'          : labels,                   # 动作实例的类别标签，长度为N，对应segments中的每个动作
            'fps'             : video_item['fps'],        # 视频帧率（frames per second），用于特征时间与原始时间的转换
            'duration'        : video_item['duration'],   # 视频总时长（单位：秒）
            'feat_stride'     : feat_stride,              # 特征步长，每个特征对应原视频的帧数（如4），用于特征时间与原始帧时间的映射
            'feat_num_frames' : self.num_frames           # 每个clip特征包含的原始帧数（如16），决定特征的时序感受野
        }
